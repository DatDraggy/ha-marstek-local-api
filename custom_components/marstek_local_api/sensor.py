...
# Venus A firmware hardcodes total_pv_energy to 0, so solar energy is
# accumulated from the derived solar power instead (persisted via RestoreSensor).
INTEGRATED_PV_ENERGY_DESCRIPTION = MarstekSensorEntityDescription(
    ...
    if isinstance(coordinator, MarstekMultiDeviceCoordinator):
        ...
        for mac in coordinator.get_device_macs():
            ...
            # Venus A reports per-string PV data instead
                ...
                # Venus A firmware never counts PV energy - integrate it
                entities.append(
                    MarstekMultiDeviceIntegratedEnergySensor(
                        coordinator=coordinator,
                        device_coordinator=device_coordinator,
                        entity_description=INTEGRATED_PV_ENERGY_DESCRIPTION,
                        device_mac=mac,
                        device_data=device_data,
                    )
                )
